...
**Status (2026-07-16): built and live on real AWS**, destroyed and rebuilt from nothing once already, both the same day — `infra/eks/` + `gitops/multi-agent/`, cluster `crag-prod-eks` confirmed `Running`, CloudFront currently live at `https://d1xqy5fg6qmir.cloudfront.net` (re-fetch before trusting, not stable across applies). See `completed.md`'s Phase 20 and Phase 21 entries and `eks-enterprize-deploy-steps.md`'s "Built vs. Designed" section for exactly where the live build diverged from the design below (notably: direct IRSA+`boto3` secret access instead of the Secrets Store CSI Driver mentioned in step 5's cross-reference, one Terraform root instead of two for CloudFront, HPA wired but not yet functional pending `metrics-server`, and — found during the from-scratch rebuild — the Stage 1 runbook's own `-target` list was missing the Internet Gateway/route table/association). **Phase 21 below is also now built and verified end-to-end** — see `completed.md`'s Phase 21 entry and `ci-cd-eks-steps.md`'s own updated Status/Gotchas for the full account, including the branch-protection/credential debugging saga (7 dispatch attempts) that turned out to be an `actions/checkout@v4` auth-header conflict, not a token problem.
