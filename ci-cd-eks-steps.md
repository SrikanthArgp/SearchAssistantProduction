...
Status: **built and verified end-to-end on real AWS (2026-07-16)** — `argocd` namespace and `multi-agent` `Application` resource live in the cluster (`kubectl get application multi-agent -n argocd` → `Synced`/`Healthy`), `.github/workflows/cd-eks.yml` exists and a real dispatch (`target: eks, environment: aws`) has gone all the way through: image build/push, bot commit to `gitops/multi-agent/values.yaml` on `main`, ArgoCD noticing and syncing, the new pod coming up healthy, and the workflow's own smoke check passing. See `completed.md`'s Phase 21 entry for the full account, including two real infra gaps found doing the from-scratch rebuild this was built alongside, and the branch-protection/credential saga below (Gotchas) that took seven dispatch attempts to resolve. This doc's own step-by-step design below is left as originally written (for the reasoning/rationale) — see the "Built vs. Designed" section right after it for where the live build actually diverged. **Note**: Phase 20's `gitops/multi-agent/` chart does not use the Secrets Store CSI Driver this doc's own reasoning was written assuming might exist alongside it — secrets are read directly via `backend_irsa` + `boto3` instead (see `eks-enterprize-deploy-steps.md`'s "Built vs. Designed" section) — this doesn't change anything in this doc's own design, since ArgoCD's job is only ever to sync `values.yaml`'s image tag, regardless of how the pod gets its secrets. Companion to [`enterprize-deploy-steps.md`](./enterprize-deploy-steps.md) (Phase 15), [`grand-enterprize-deploy-steps.md`](./grand-enterprize-deploy-steps.md) (Phase 16), [`eks-enterprize-deploy-steps.md`](./eks-enterprize-deploy-steps.md) (Phase 20), [`cd-lambda-deploy-steps.md`](./cd-lambda-deploy-steps.md) (Phase 18), [`cd-ecs-deploy-steps.md`](./cd-ecs-deploy-steps.md) (Phase 19), and [`cd-dispatcher-steps.md`](./cd-dispatcher-steps.md) — read the Phase 18/19 docs and the dispatcher doc first, since this doc only calls out where the GitOps shape *differs* from their direct-push pattern, not where it's the same.

---

## Built vs. Designed

Real deviations from this doc's own design below, found building and verifying it end-to-end on real AWS (2026-07-16) — read alongside the step-by-step design, not instead of it:

1. **The deploy role is much broader than "ECR push + repo content-write only."** Step 4/the architecture diagram both describe `cd_eks_deploy` as needing no `eks:*`/`kubectl`-equivalent permission at all. The live role (`infra/bootstrap/github-oidc.tf`) grants `eks:*`, `ec2:*`, `elasticloadbalancing:*`, `cloudfront:*`, full management of this stack's 4 IAM roles, and Terraform-state S3/DynamoDB access — matching `cd-lambda-deploy-role`/`cd-ecs-deploy-role`'s own "fast-path vs. full-apply" shape instead, per explicit instruction to mirror those two workflows' structure exactly. `cd-eks.yml` does run a conditional `terraform apply` against `infra/eks/` when `infra/eks/**` changed (same `paths-filter` pattern as `cd-lambda.yml`/`cd-ecs.yml`), which the original "ECR push only" design never accounted for — the role needs to be able to do everything a full `infra/eks` apply might touch.
2. **The `Application`'s destination namespace is `default`, not `multi-agent`.** Step 3's sample manifest uses a dedicated `multi-agent` namespace. The live build's `argocd/multi-agent-application.yaml` uses `default`, matching what `eks-manual-deploy-steps.md`'s own Stage 5 (`helm upgrade --install multi-agent gitops/multi-agent -n default ...`) actually installs into — the sample manifest here was never updated to match once that runbook's own namespace choice was made.
3. **The bot-commit push needs a PAT (`BOT_PUSH_TOKEN` secret), not the default `GITHUB_TOKEN`.** Step 9/the workflow YAML both assume `permissions: contents: write` on `GITHUB_TOKEN` is sufficient. It isn't, for this specific repo: `main`'s branch protection rejects a direct push from `GITHUB_TOKEN` outright (`GH006`, required status checks), and `GITHUB_TOKEN` isn't an admin-bypass identity. See the Gotchas section below for the full, non-obvious debugging path this took (a ruleset bypass attempt rejected by a real GitHub platform limitation, then a real `actions/checkout@v4` credential-conflict bug that made six different token attempts all fail identically for the same underlying reason).
4. **The `Application` was created against a fresh, from-scratch EKS rebuild, not the original Phase 20 stack.** Step 4's "adopt, don't duplicate" concern (Phase 20's manual `helm install` already existing) was real, but the specific Phase 20 stack this doc was originally written against had already been destroyed and rebuilt from nothing earlier the same session — the "existing manual release" being adopted was the fresh rebuild's own Stage 5 `helm install`, not a long-lived one. The handoff procedure itself (uninstall, then let the `Application`'s first sync create cleanly) worked as designed regardless.
...
- **A branch-protection rule on `main` requiring PR review will reject this workflow's direct push outright — confirmed for real, 2026-07-16, and it took seven dispatch attempts to actually fix.** `git push` from `GITHUB_TOKEN` is a direct push, not a PR — `main`'s required-status-checks protection (`enforce_admins: false`) rejects it with `GH006: Protected branch update failed... 2 of 2 required status checks are expected`, since `GITHUB_TOKEN` isn't an admin-bypass identity. The obvious-looking fixes, in the order actually tried, and why each one *appeared* to fail:
  - **Converting to a GitHub ruleset with GitHub Actions as an explicit bypass actor** — rejected outright by the API: `"Actor GitHub Actions integration must be part of the ruleset source or owner organization"`. This bypass mechanism only works for organization-owned repos; a personal-account repo (like this one) can't use it at all, full stop.
  - **A repo-scoped PAT (`BOT_PUSH_TOKEN` secret)**, used via `git push https://x-access-token:$BOT_PUSH_TOKEN@github.com/...` — failed with the *identical* `GH006` error across four further attempts (empty secret value from a non-interactive `gh secret set` prompt; a fine-grained PAT scoped to `Contents: read/write`; the same PAT broadened to add `Administration: read/write`; a classic full-`repo`-scope PAT, twice). Every one of these looked like "the token still isn't right," but **none of them were the actual problem**: a controlled test (pushing with an already-proven-working token in the exact same URL format, from outside Actions) succeeded immediately, proving the push mechanism itself was fine.
  - **The real cause**: `actions/checkout@v4` configures `git config --local http.https://github.com/.extraheader "AUTHORIZATION: basic ***"` (a `GITHUB_TOKEN`-derived header), and this config **persists in the job's git state for the entire job** — it's only unset in the job's own automatic "Post Checkout" cleanup step, which runs *after* the push already happened. Git sends that leftover header on every subsequent request to `github.com`, and it silently wins over whatever credential is embedded directly in a push URL — meaning **every one of the six credential-swap attempts above was still actually authenticating as `GITHUB_TOKEN`, never the PAT, regardless of what was in the secret.** The fix is one line, run immediately before the push: `git config --local --unset-all http.https://github.com/.extraheader || true`. This is a real, general gotcha for *any* workflow that checks out with `actions/checkout@v4`'s defaults and then tries to push with a different credential later in the same job — not specific to this repo or this PAT.
  - See `completed.md`'s Phase 21 entry for the full attempt-by-attempt account.
