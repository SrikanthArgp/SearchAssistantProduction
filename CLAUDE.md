...
- `plan.md` — full target design: new folder structure, schema, API endpoints, phase-by-phase migration order (Phases 1–14), plus a follow-on enterprise-grade AWS deployment + CI/CD pass (Phases 15–19, design-only, not scheduled until 1–14 are absorbed), plus a further additive EKS + ArgoCD/GitOps pass (Phases 20–21, design-only, added 2026-07-09 — its original gating condition, Phases 15–19 being absorbed, was met 2026-07-13 when both Stage Cs closed, so as of 2026-07-15 it's unblocked and ready to schedule whenever, not waiting on anything else)
  ...
  - **Phase 20 (EKS) is now built and live on real AWS (2026-07-16)** — `infra/eks/` (own VPC, cluster `crag-prod-eks`, node group, IRSA) + a new `gitops/multi-agent/` Helm chart, the AWS Load Balancer Controller provisioning a real ALB from the chart's `Ingress`, and its own CloudFront distribution + S3 bucket, confirmed live end-to-end (login/chat flow) at `https://d3m8imkry3vkn4.cloudfront.net` — **note this CloudFront domain is not stable across applies, same as every other real-AWS stack in this project; re-fetch it rather than trusting this value if it's been a while.** The live build has four confirmed deviations from `eks-enterprize-deploy-steps.md`'s original design, found by inspecting the running cluster directly: secrets are read via direct IRSA + `boto3` (the same `config.py bootstrap_env()` path Lambda/Fargate already use), not the Secrets Store CSI Driver the design doc called for; CloudFront lives in the same Terraform root as the cluster, not a second config; the frontend S3 bucket is its own dedicated bucket, not reused from Phase 15; and the `HorizontalPodAutoscaler` is wired but not yet functional since no `metrics-server` is installed in-cluster. See `completed.md`'s Phase 20 entry and `eks-enterprize-deploy-steps.md`'s "Built vs. Designed" section for the full list, including an as-designed vs. as-built architecture diagram pair. A third recurrence of the MSYS/Git-Bash frontend-build path-mangling bug (`MSYS_NO_PATHCONV=1` needed when building the frontend static export, or `/v1/...` API paths get baked in as mangled `C:/Program Files/Git/...` paths) hit this stack too and was fixed the same way as Phases 15/16. **Phase 20 was destroyed later the same day (2026-07-16) at explicit user request, then rebuilt from nothing and re-verified in a second session the same day** — a fresh `terraform apply` from an empty state, not a resume, surfaced one genuine gap `eks-manual-deploy-steps.md`'s Stage 1 target list had never hit before: it targets `aws_eks_node_group.this` assuming Terraform's `-target` dependency closure pulls in "VPC, subnets, IGW, route table" automatically, but the IGW/route-table/route-table-association are never actually referenced by the node group or subnet resources, so they weren't created — the node had a public IP but no route out, and node-group creation failed after ~25 min (`NodeCreationFailure`, confirmed via `nodeadm`'s console log retrying `EC2/DescribeInstances` forever). Fixed and documented in that runbook. **Phase 21 (ArgoCD/GitOps CD for the EKS target) is now built and verified end-to-end on real AWS (2026-07-16)** — `.github/workflows/cd-eks.yml`, ArgoCD installed in-cluster managing a `multi-agent` `Application` (`argocd/multi-agent-application.yaml`, `Synced`/`Healthy`), `cd.yml`'s dispatcher wired with an `eks`/`all` target, and `infra/bootstrap`'s `cd-eks-deploy-role` applied. Two more real gaps found doing this: (1) ArgoCD's first sync (after the required `helm uninstall` handoff from Phase 20's manual release) makes the AWS Load Balancer Controller provision a **brand-new physical ALB**, leaving CloudFront's origin stale (`502`) until a plain `terraform apply` re-resolves `data "aws_lb"` — documented in `ci-cd-eks-steps.md`'s Gotchas; (2) the bot-commit push to `main` needed real credential debugging — `GITHUB_TOKEN` can't bypass this repo's branch protection at all (not an admin identity), a GitHub ruleset bypass-actor was tried and rejected outright since that mechanism only works for org-owned repos (this is a personal account), and even after switching to a repo-scoped PAT (`BOT_PUSH_TOKEN` secret) the push kept failing identically across several credential changes (fine-grained → +Administration → classic full-`repo`-scope) until the actual cause was found: `actions/checkout@v4` leaves a `GITHUB_TOKEN`-derived `http.extraheader` active in git config for the whole job, silently overriding whatever token was embedded in the push URL — every credential attempt before the fix was still authenticating as `GITHUB_TOKEN` regardless. One line clearing that header before the push resolved it. See `completed.md`'s Phase 21 entry for the full account and `ci-cd-eks-steps.md` for the as-built design (both docs updated 2026-07-16).
