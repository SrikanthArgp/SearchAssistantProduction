...
- `plan.md` — full target design: new folder structure, schema, API endpoints, phase-by-phase migration order (Phases 1–15), plus a follow-on enterprise-grade AWS deployment pass (Phases 16–17, design-only, not scheduled until 1–15 are absorbed)
  - Phases 1–4 are already **built and tested**; Phases 5–15 (including `config.py`, `/v1` API versioning, global error handling, auth-specific rate limiting, a Next.js frontend split across Phase 7 (auth) and Phase 8 (chat UI), and full-stack Dockerization in Phase 10) are still design-only in `plan.md`, not present in the current code
